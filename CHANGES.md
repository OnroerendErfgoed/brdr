...
- Added `processing_area` behavior across processors
- Added DirectedNetworkGeometryProcessor
- Enum and remark updates:
    - renamed `INVALID_PRERESULT_ORIGINAL_RETURNED` to `INVALID_EMPTY_RETURNED`
    - removed unused `ProcessRemark` values and aligned usage with code paths
- Documentation updates:
    - synchronized `Q&A` and reference docs with current empty/original return behavior
    - updated processor and config reference pages
    - removed outdated/duplicate metrics explanation section in docs
- Closed tickets in GitHub milestone `0.17.0`:
    - [#343] NetworkGeometryProcessor: when reconstructing polygon, check that main part of polygon is in result
