...
- Short answer:
  - `original geometry` is returned for safe fallbacks and unchanged outcomes.
  - `empty geometry` is only returned in specific valid/explicit situations.
- Long answer:
  - Typical `original geometry` cases:
    - no effective change after processing
    - circular-shape guard behavior
    - invalid intermediate processor output (strict fallback to original)
...
- `INVALID_PRERESULT_ORIGINAL_RETURNED`:
  - processor intermediate output was unusable (e.g. invalid/empty/type mismatch path);
    `brdr` returned original geometry as a strict safe fallback.
- `RESULT_EMPTY_RETURNED` / `CHANGED_GEOMETRYTYPE_EMPTY_RETURNED`:
  - legacy/diagnostic remarks indicating an empty/type-mismatch outcome path.
    In current strict-fallback behavior, these should be much less common.
